...
int main() {
    ...
    image_p2* tmp9 = image_reset(tmp7->M, tmp7->N, tmp7->scale, tmp7->row_size, tmp7->col_size);//함수에 넣을 출력입니다.
    preHys(tmp8,tmp9,tmp7,tmp4);
    image_write("result08.pgm", tmp8);
    image_write("result09.pgm", tmp9);

    
    image_p2* tmp10= image_reset(tmp7->M, tmp7->N, tmp7->scale, tmp7->row_size, tmp7->col_size);//함수에 넣을 출력입니다.
    Hys(tmp10, tmp7, tmp4);
